...
\begin{itemize}
    ...
    \item \texttt{game\_archive}: Stores a chronological list of snapshots containing both characters' health, skill points, classes, and the player's inventory at the start of each player turn.
\end{itemize}
...
Skill points (SP) are utilized to cast skills which are unique to every class. There methods of regeneration of skill points for each class is as follows:
\begin{itemize}
...
\subsection{Defense}
The stat "Defense" is different from the action "Defend", do be mindful of this. The action "Defend" decreases the amount of damage taken by the character upon recieving damage during the opponent's turn. The stat "Defense" refers to the probability of a character to dodge an attack, recieving no damage from it. The stat is unique to each class and goes as follows:
\begin{itemize}
